...
const Who: React.FC<WhoProps> = ({ who = [] }) => {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-2xl font-bold text-[#1D293D] mb-4 relative inline-block">
          <span className="relative z-10">
            {whoData.heading || "Why Choose Corpbiz for FSSAI State Food License Registration?"}
          </span>
          <motion.span 
            initial={{ scaleX: 0 }}
            animate={inView ? { 
      ...
      {/* Description Section */}
      {whoData.description?.map((paragraph, index) => (
        <motion.p
          key={index}
          initial={{ opacity: 0, y: 10 }}
          animate={inView ? { 
      ...
      {/* Points Section */}
      {whoData.points && whoData.points.length > 0 && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={inView ? { 
            opacity: 1,
        ...
        >
          {whoData.points.map((point, index) => (
            <motion.div
              key={index}
      ...
      {whoData.footerText && (
        <motion.p
          ...
          animate={inView ? { 
            opacity: 1,
            transition: { delay: 0.6 + (whoData.points?.length || 0) * 0.1 }
          } : { opacity: 0 }}
